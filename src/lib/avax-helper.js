...
const maxGas = parseInt(process.env.GASLIMIT_AVAX);
const maxGasPrice = parseInt(process.env.GASPRICE_AVAX)
async function signTx(gasLimit, nonce, data, prvKey, value, to, _gasPrice) {
  ...
  const txParams = {
    ...
    value: value,
    data: data,
    chainId: web3.utils.toHex(parseInt(process.env.CHAIN_ID_AVAX)),
  };
