...
const maxGas = parseInt(process.env.GASLIMIT_BSC);
const maxGasPrice = parseInt(process.env.GASPRICE_BSC)
async function signTx(gasLimit, nonce, data, prvKey, value, to, _gasPrice) {
  ...
  const txParams = {
    ...
    value: value,
    data: data,
    chainId: web3.utils.toHex(parseInt(process.env.CHAIN_ID_BSC)),
  };
